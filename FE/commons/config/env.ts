...
export const env: EnvConfig = {
  // 백엔드 API
  apiBaseUrl: getEnvValue('EXPO_PUBLIC_API_BASE_URL') || 'http://localhost:3000',
  ...
  // 앱 설정
  appEnv: (getEnvValue('EXPO_PUBLIC_APP_ENV') || 'development') as EnvConfig['appEnv'],
...
if (__DEV__ && env.debugMode) {
  ...
  console.log(`  - Kakao Map API Key: ${env.kakaoMapApiKey ? '✅ 설정됨' : '❌ 미설정'}`);
